...
export function useComponentOptionsActions() {
  ...
  return {
    ...
    createComponentOption(componentOption) {
      loading.value = true
      return axios.post(`http://dssspecs-backend-ibiz5.ondigitalocean.app/api/component-options`, { data:  componentOption }).then((res) => {
    ...
    updateComponentOption(componentOption) {
      loading.value = true
      const promise = axios.put(`http://dssspecs-backend-ibiz5.ondigitalocean.app/api/component-options/${componentOption.id}`, { data: componentOption }).then(() => {
        queryClient.invalidateQueries({ queryKey: ['component-options'] })
      }).finally(() => {
        loading.value = false
      })
